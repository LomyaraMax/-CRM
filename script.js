...
// Визначити колір рамки по дедлайну
function getDeadlineColor(deadline) {
  if (!deadline) return "#ccc"; // сірий якщо немає дати
  const now = new Date();
  const target = new Date(deadline);
  const diffHours = (target - now) / (1000 * 60 * 60);

  if (diffHours > 72) return "green";      // більше 3 днів
  if (diffHours > 24) return "yellow";     // менше 3 днів
  if (diffHours > 0) return "red";         // менше доби
  return "black";                          // прострочено
}
...
async function renderClients() {
  ...
  for (const docSnap of snapshot.docs) {
    ...
    // Додаємо колір рамки
    card.style.border = `3px solid ${getDeadlineColor(data.deadline)}`;
